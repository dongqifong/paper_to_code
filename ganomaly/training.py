...
class Trainer():
    def __init__(self,generator,discriminator,data_source_path,columns_name,batch_size=1,shuffle=False) -> None:
        self.generator = generator
        self.optimizer_g = torch.optim.Adam(self.generator.parameters(),lr=1e-4)
        self.discriminator = discriminator
        self.optimizer_d = torch.optim.Adam(self.discriminator.parameters(),lr=1e-6)
        ...
        self.g_loss_list = []
        self.d_loss_list = []
        self.dataset = ModelDataset(self.data_source, columns_name=self.columns_name)
        self.dataloader = get_dataloader(self.dataset,batch_size=self.batch_size,shuffle=self.shuffle)
    ...
    def train_d_one_epoch(self):
        ...
        for batch_idx, x in enumerate(self.dataloader):
            ...
            label_real = torch.full((z1.shape[0],), self.real_label, dtype=torch.float, device=self.device)
            label_fake = torch.full((z2.shape[0],), self.fake_label, dtype=torch.float, device=self.device)

            loss = self.d_loss_fucn(torch.concat([y_z1, y_z2]), torch.concat([label_real, label_fake]))
            loss.backward()
...
if __name__ == "__main__":
    ...
    dataset = ModelDataset(data_source_path, columns_name=["a"])
    x, path = dataset.__getitem__(0)
    xx = torch.concat([x,x])
    label = torch.full((3,), 0)
    label
    label.fill_(1)